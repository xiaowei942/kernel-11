			...
			unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		...
		case GETBUFFNUM:
			...
			{
				printk (KERN_ALERT "petworm: here add code.\n");
				printk (KERN_ALERT "read_head is %x.\n", tty->read_head);
				printk (KERN_ALERT "read_tail is %x.\n", tty->read_tail);
				printk (KERN_ALERT "read_cnt is %x.\n", tty->read_cnt);
			}
#endif
			if (tty->read_cnt == 0)
				cbe = 0;
			else if (tty->read_cnt >= 0xfff)
				cbe = 1;
			else 
				cbe = 2;

			if (copy_to_user((void *) p, (void *) &cbe, 1))
