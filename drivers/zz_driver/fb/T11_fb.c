...
void Lcd_Init(void)
{
	...
	*(p++) = 0x10001003;
	...
	/* read LCD chip id */
	//MainLCD_Command(0x0000); MainLCD_Read(&tmp);
	//_SetILI9325 (tmp);

	printk (KERN_ALERT "petworm: sIsILI9325 is %x.\n", _GetILI9325());
#if 0
	if (_GetILI9325())
	{
...
*	返回值：void
******************************************************************************/
void fb_entry_sleep_9320 (unsigned short int cmd)
{
	/*07 寄存器,关闭显示 */
...
*	返回值：void
**********************************************************************************/
void fb_exit_sleep_9320(unsigned short int cmd)
{
	int i;
...
*	返回值：void
******************************************************************************/
void fb_entry_sleep_9325 (unsigned short int cmd)
{
	MainLCD_Command(0x0007); MainLCD_Data(0x0131); // Set D1=0, D0=1
	Delayms(10);
	MainLCD_Command(0x0007); MainLCD_Data(0x0130); // Set D1=0, D0=0
	Delayms(10);
	MainLCD_Command(0x0007); MainLCD_Data(0x0000); // display OFF
	//************* Power OFF sequence **************//
	MainLCD_Command(0x0010); MainLCD_Data(0x0080); // SAP, BT[3:0], APE, AP, DSTB, SLP
	MainLCD_Command(0x0011); MainLCD_Data(0x0000); // DC1[2:0], DC0[2:0], VC[2:0]
	...
	MainLCD_Command(0x0010); MainLCD_Data(0x0082); // SAP, BT[3:0], APE, AP, DSTB, SLP
}
...
*	返回值：void
**********************************************************************************/
void fb_exit_sleep_9325(unsigned short int cmd)
{
	//*************Power On sequence ******************//
	MainLCD_Command(0x0010); MainLCD_Data(0x0080); // SAP, BT[3:0], AP, DSTB, SLP
	MainLCD_Command(0x0011); MainLCD_Data(0x0000); // DC1[2:0], DC0[2:0], VC[2:0]
	MainLCD_Command(0x0012); MainLCD_Data(0x0000); // VREG1OUT voltage
	MainLCD_Command(0x0013); MainLCD_Data(0x0000); // VDV[4:0] for VCOM amplitude
	Delayms(200); 
	MainLCD_Command(0x0010); MainLCD_Data(0x1490); // SAP, BT[3:0], AP, DSTB, SLP, STB
	MainLCD_Command(0x0011); MainLCD_Data(0x0227); // DC1[2:0], DC0[2:0], VC[2:0]
	Delayms(50); // Delay 50ms
	MainLCD_Command(0x0012); MainLCD_Data(0x001A); // Internal reference voltage= Vci;
	Delayms(50); // Delay 50ms
	MainLCD_Command(0x0013); MainLCD_Data(0x1400); // Set VDV[4:0] for VCOM amplitude
	MainLCD_Command(0x0029); MainLCD_Data(0x0020); // Set VCM[5:0] for VCOMH
	Delayms(50); // Delay 50ms
	MainLCD_Command(0x0007); MainLCD_Data(0x0133); // 262K color and display ON

	// ili9325 regsters initial
	// Modified by Santiage 2011017
	/************* Start Initial Sequence **********/
	MainLCD_Command(0x00E3); MainLCD_Data(0x3008);
	MainLCD_Command(0x00E7); MainLCD_Data(0x0012);
	MainLCD_Command(0x00EF); MainLCD_Data(0x1231);
	MainLCD_Command(0x0001); MainLCD_Data(0x0100); // set SS and SM bit
	MainLCD_Command(0x0002); MainLCD_Data(0x0700); // set 1 line inversion
	MainLCD_Command(0x0003); MainLCD_Data(0x1030); // set GRAM write direction and BGR=1.
	MainLCD_Command(0x0004); MainLCD_Data(0x0000); // Resize register
	MainLCD_Command(0x0008); MainLCD_Data(0x0207); // set the back porch and front porch
	MainLCD_Command(0x0009); MainLCD_Data(0x0000); // set non-display area refresh cycle ISC[3:0]
	MainLCD_Command(0x000A); MainLCD_Data(0x0000); // FMARK function
	MainLCD_Command(0x000C); MainLCD_Data(0x0000); // RGB interface setting
	MainLCD_Command(0x000D); MainLCD_Data(0x0000); // Frame marker Position
	MainLCD_Command(0x000F); MainLCD_Data(0x0000); // RGB interface polarity
	//*************Power On sequence ****************//
	MainLCD_Command(0x0010); MainLCD_Data(0x0000); // SAP, BT[3:0], AP, DSTB, SLP, STB
	...
	MainLCD_Command(0x0013); MainLCD_Data(0x1400); // Set VDV[4:0] for VCOM amplitude
	MainLCD_Command(0x0029); MainLCD_Data(0x0020); // Set VCM[5:0] for VCOMH
	MainLCD_Command(0x002B); MainLCD_Data(0x000D); // Set Frame Rate
	Delayms(50); // Delay 50ms
	MainLCD_Command(0x0020); MainLCD_Data(0x0000); // GRAM horizontal Address
	MainLCD_Command(0x0021); MainLCD_Data(0x0000); // GRAM Vertical Address
	// ----------- Adjust the Gamma Curve ----------//
	MainLCD_Command(0x0030); MainLCD_Data(0x0000);
	MainLCD_Command(0x0031); MainLCD_Data(0x0604);
	MainLCD_Command(0x0032); MainLCD_Data(0x0306);
	MainLCD_Command(0x0035); MainLCD_Data(0x0204);
	MainLCD_Command(0x0036); MainLCD_Data(0x1604);
	MainLCD_Command(0x0037); MainLCD_Data(0x0104);
	MainLCD_Command(0x0038); MainLCD_Data(0x0301);
	MainLCD_Command(0x0039); MainLCD_Data(0x0707);
	MainLCD_Command(0x003C); MainLCD_Data(0x0402);
	MainLCD_Command(0x003D); MainLCD_Data(0x000F);
	//------------------ Set GRAM area ---------------//
	MainLCD_Command(0x0050); MainLCD_Data(0x0000); // Horizontal GRAM Start Address
	MainLCD_Command(0x0051); MainLCD_Data(0x00EF); // Horizontal GRAM End Address
	MainLCD_Command(0x0052); MainLCD_Data(0x0000); // Vertical GRAM Start Address
	MainLCD_Command(0x0053); MainLCD_Data(0x013F); // Vertical GRAM Start Address
	MainLCD_Command(0x0060); MainLCD_Data(0xA700); // Gate Scan Line
	MainLCD_Command(0x0061); MainLCD_Data(0x0001); // NDL,VLE, REV
	MainLCD_Command(0x006A); MainLCD_Data(0x0000); // set scrolling line
	//-------------- Partial Display Control ---------//
	MainLCD_Command(0x0080); MainLCD_Data(0x0000);
	MainLCD_Command(0x0081); MainLCD_Data(0x0000);
	MainLCD_Command(0x0082); MainLCD_Data(0x0000);
	MainLCD_Command(0x0083); MainLCD_Data(0x0000);
	MainLCD_Command(0x0084); MainLCD_Data(0x0000);
	MainLCD_Command(0x0085); MainLCD_Data(0x0000);
	//-------------- Panel Control -------------------//
	MainLCD_Command(0x0090); MainLCD_Data(0x0010);
	MainLCD_Command(0x0092); MainLCD_Data(0x0600);
	MainLCD_Command(0x0007); MainLCD_Data(0x0133); // 262K color and display ON
}
#endif
...
{
	if (_GetILI9325())
		fb_entry_sleep_9325(cmd);
	else
		fb_entry_sleep_9320(cmd);
}

void fb_exit_sleep(unsigned short int cmd)
{
	if (_GetILI9325())
		fb_exit_sleep_9325(cmd);
	else
		fb_exit_sleep_9320(cmd);
}
