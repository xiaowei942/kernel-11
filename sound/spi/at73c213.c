...
#if 0	/* Santiage off because there is not spi any more */
...
/*
 * set CS4344 volume: 1 - 高；0 - 低
 */
/*
static int snd_CS4344_vol_set(int vol)
{

	//if (vol) vol = 1;

	at91_set_gpio_value (AT91_PIN_PB17, 0);
	at91_set_gpio_value (AT91_PIN_PB21, vol);
	at91_set_gpio_value (AT91_PIN_PB19, 0);
	udelay (100);
	at91_set_gpio_value (AT91_PIN_PB19, 1);
...
static int snd_at73c213_pcm_open(struct snd_pcm_substream *substream)
{
	...
	/* Santiage add for enable the CS4344 chip */
#ifdef CS4344_ALL
	printk ("######################################################\n");
	printk ("petworm: ALSA open.\n");
	printk ("######################################################\n");
	printk (KERN_ALERT "petworm: CS4344: open.\n");
	at91_set_gpio_value (AT91_PIN_PB24, 1);
	at91_sys_write (AT91_PMC_PCER, 0x1 << 15);
#endif
	/* end */
...
static int snd_at73c213_pcm_close(struct snd_pcm_substream *substream)
{
	...
	/* petorm add for disable CS4344 chip */
#ifdef CS4344_ALL
	printk ("######################################################\n");
	printk ("petworm: ALSA close.\n");
	printk ("######################################################\n");
	printk (KERN_ALERT "petworm: CS4344: close.\n");
	at91_set_gpio_value (AT91_PIN_PB24, 0);

	at91_sys_write (AT91_PMC_PCDR, 0x1 << 15);
#endif
	/* end */
	chip->substream = NULL;
	return 0;
				 ...
				 struct snd_ctl_elem_value *ucontrol)
{
	...
	printk (KERN_ALERT "petworm: mono get.\n");
				 ...
				 struct snd_ctl_elem_value *ucontrol)
{
	...
	printk (KERN_ALERT "petworm: mono put.\n");
...
#ifndef CS4344_ALL
	/* Santiage add for there is not spi any more */
	retval = snd_at73c213_write_reg(chip, reg, val);
#else
	//retval = snd_CS4344_vol_set(val);
				 ...
				 struct snd_ctl_elem_value *ucontrol)
{
	...
	printk (KERN_ALERT "petworm: stereo get.\n");
	spin_lock_irq(&chip->lock);
				 ...
				 struct snd_ctl_elem_value *ucontrol)
{
	...
	printk (KERN_ALERT "peworm val1 is %d,val2 is %d\n", val1, val2);
#if 0
	if (invert) {
		val1 = mask - val1;
		val2 = mask - val2;
...
#else	/* write PIOB21 control the vol */
	//retval = snd_CS4344_vol_set(val1);
#endif
	spin_unlock_irq(&chip->lock);
...
/*
	for (i = 0; i < 4; i++)
	{
		at91_set_gpio_output (piob_pin[0][i], 1);
		at91_set_gpio_value (piob_pin[0][i],piob_pin[1][i]);
	}

	for (i = 0; i < 32; i++)
	{
		snd_CS4344_vol_set(1);
	}
*/
#endif
	/* end */
...
static int __init at73c213_init(void)
{
	...
	printk ("#################################################\n");
	printk ("petworm: probe return. set pmc.\n");
	printk ("################################################\n");
	at91_sys_write (AT91_PMC_PCDR, 0x1 << 15);
