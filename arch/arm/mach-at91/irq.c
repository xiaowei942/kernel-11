...
/*
	if (irq == 0x1)
		printk (KERN_ALERT"SYSIRQ already set.\n");
*/
	if (unlikely(irq >= 32))
		return -EINVAL;
...
void at91_irq_suspend(void)
{
	...
	//printk (KERN_ALERT "petworm: irq_suspend: backups is %x, wakeups is %x.\n", backups, wakeups);
...
void at91_irq_resume(void)
{
	...
	//printk (KERN_ALERT "petworm: irq_resume: backups is %x, wakeups is %x.\n", backups, wakeups);
}
...
void __init at91_aic_init(unsigned int priority[NR_AIC_IRQS])
{
	...
	//wakeups |= (0x1 << 30);
	wakeups |= (0x1 << 1);
	printk (KERN_ALERT "petworm: wakeups is %x.\n", wakeups);
#endif
	/*
